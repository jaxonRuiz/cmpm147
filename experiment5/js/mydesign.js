...
function getInspirations() {
  ...
  function mutateDesign(design, inspiration, rate) {
    ...
    for(let [colorName, boxes] of Object.entries(design.colors)){
      for(let box of boxes) {
        ...
        box.colorVal = mut(box.colorVal, 0, 255, rate);
        box.alpha = mut(box.alpha, 0, 255, rate);

        box.x = mut(box.x, 0, width, rate);
        box.y = mut(box.y, 0, height, rate);
        box.w = mut(box.w, 0, width*3/4, rate);
        box.h = mut(box.h, 0, height*3/4, rate);
      }
    }
  }
  ...
  function isBetter(num, min, max, rate) {
